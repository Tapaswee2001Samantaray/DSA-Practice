...
class DoublyLinkedList {
    constructor(urlValue, next, previous) {
        this.urlValue = urlValue == undefined ? 0 : urlValue;
...
var BrowserHistory = function (homepage) {
    this.head = new DoublyLinkedList(homepage);
    this.current = this.head;
}

var visit = function (url) {
    this.node = new DoublyLinkedList(url);
    this.node.previous = this.current;
    this.current.next = this.node;
    this.current = this.current.next;
}

var back = function (steps) {
    while (steps > 0 && this.current.previous != null) {
        this.current = this.current.previous;
        steps--;
    }
    return this.current.urlValue;
}
