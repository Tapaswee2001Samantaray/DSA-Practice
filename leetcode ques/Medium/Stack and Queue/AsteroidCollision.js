...
var asteroidCollision = function (asteroids) {
    ...
    for (let i = 0; i < asteroids.length; i++) {
        if (asteroids[i] > 0) {
            ...
            while (stack.length != 0 && stack[stack.length - 1] > 0 && stack[stack.length - 1] < -asteroids[i]) {
                stack.pop();
            }
...
// let a=asteroidCollision([5,10,-5]);
// let a=asteroidCollision([2,2,-5]);
let a = asteroidCollision([-2, 2, -2, 2]);
console.log(a);