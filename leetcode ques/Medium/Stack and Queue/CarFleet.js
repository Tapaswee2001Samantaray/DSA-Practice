...
A car can never pass another car ahead of it, but it can catch up to it and drive bumper to bumper at the same speed. The 
faster car will slow down to match the slower car's speed. The distance between these two cars is ignored 
...
var carFleet = function (target, position, speed) {
    ...
    let timeTaken = new Array(n);

    let arrOfObjects = [];

    for (let i = 0; i < n; i++) {
        timeTaken[i] = (target - position[i]) / speed[i];
        arrOfObjects.push({
            position: position[i],
    ...
    arrOfObjects.sort((a, b) => a.position - b.position);

    let stack = [];
    stack.push(arrOfObjects[n - 1].timeTaken);

    for (let i = n - 2; i >= 0; i--) {
        if (arrOfObjects[i].timeTaken > stack[stack.length - 1]) {
            stack.push(arrOfObjects[i].timeTaken);
        }
    }
    return stack.length;
}

let a = carFleet(12, [10, 8, 0, 5, 3], [2, 4, 1, 1, 3]);
console.log(a);