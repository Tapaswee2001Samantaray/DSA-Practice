...
You are also given an integer totalTrips, which denotes the number of trips all buses should make in total. Return the minimum time 
...
let calculateTripsTaken = function (mid, time) {
    let tripCount = 0;
    for (let i = 0; i < time.length; i++) {
        tripCount += Math.floor(mid / time[i]);
    }
...
var minimumTime = function (time, totalTrips) {
    ...
    while (low <= high) {
        ...
        let totalTripCompleted = calculateTripsTaken(mid, time);

        if (totalTripCompleted >= totalTrips) {
            minTimeTaken = mid;
            high = mid - 1;
        } else {
            low = mid + 1;
...
let a = minimumTime([1, 2, 3], 5);
// let a = minimumTime([2], 1);
