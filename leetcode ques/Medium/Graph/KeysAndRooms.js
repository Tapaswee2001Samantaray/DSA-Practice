...
var canVisitAllRooms = function (rooms) {
    ...
    while (stack.length != 0) {
        let node = stack.pop();
        let countOfNode = rooms[node].length;

        for (let i = 0; i < countOfNode; i++) {
            if (visitedArr[rooms[node][i]] == false) {
                stack.push(rooms[node][i]);
                visitedArr[rooms[node][i]] = true;
            }
        }
    }
...
// let a = canVisitAllRooms([[1], [2], [3], []]);
let a = canVisitAllRooms([[1, 3], [3, 0, 1], [2], [0]]);
console.log(a);